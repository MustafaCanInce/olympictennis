...
public class Player_2 : MonoBehaviour{
    ...
    void Update(){
        ...
        if (Input.GetKeyDown(KeyCode.F)){
            hitting = true; // Objective is to strike the ball and determine where it should land.
        ...
        }                    // This is utilized to alternate between moving the aiming target and our own position.
        ...
        if ((Input.GetKey(KeyCode.DownArrow) || Input.GetKey(KeyCode.UpArrow)) && !hitting){ // Restrict the movement of the aiming gameObject to only forward or backward.
            transform.Translate(new Vector3(0, 0, v) * speed * Time.deltaTime); // Move on the court.
        }

        if ((Input.GetKey(KeyCode.RightArrow) || Input.GetKey(KeyCode.LeftArrow)) && !hitting){ // Restrict the movement of the aiming gameObject to only left or right.
